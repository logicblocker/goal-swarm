# goal-swarm's Agent Protocol Implementation Notes

> `AP=Agent Protocol` <br /> context: [OpenAPI](/apps/nextjs/lib/AgentProtocol/openapi.json)

> `GS=goal-swarm` <br /> context: [Prisma Schema](/packages/db/prisma/schema.prisma)
> (OpenAPI coming soon)

## AP↔GS Mappings

- AP:Task ~= GS:(Goal+Execution)
- AP:Step ~= GS:ExecutionNode
- AP:Artifact ~= GS:Result
...
// GS: returns executions, may return executions from other Goals (which are effectively other Agents)
```

AP: assumes there can only be one Agent in a system. Isolation would currently have to be done w/ `additional_input`

## AP↔GS Incongruencies
...
- Scoping
  ...
  - GS: uses the `Goal` container to share some common configuration, such as the `Goal` prompt, (and planned:) certain types of persistent data, `Skill` access, etc.
  - GS: `namespace` (e.g. `xyz`, `xyz_1.1`, `zyx`) is used to scope memory access,
  - GS: `Skills` are planned to require certain user-configurable permissions. e.g. temporary code execution, requires human intervention, read-only, risk-based, etc.

- Parallelism:
  - GS: Parallelism is possible both by creating multiple `Execution` and also possible with the structure of `ExecutionGraph`.
  - GS: When conforming to AP, parallelism is currently reduced to serial. 👎
  - AP: parallelism has to be implemented in another layer, or possibly(?) with `additional_input`

Opinions ([@logicblocker](https://github.com/logicblocker)):
